...
export const getOrders = (context: any) => {
  return async (dispatch: Dispatch<OrderAction | NotifyAction>) => {
    try {
      const { ozonaToken } = parseCookies(context);
      const response = await getAPI('order', ozonaToken);
      dispatch({ type: OrderActionTypes.GET_ORDERS, payload: response.data });
    } catch (error: any) {
      dispatch({
        type: NotifyActionTypes.NOTIFY,
        payload: { errors: error.response.data.message },
...
export const deliveredOrder = (id: number) => {
  return async (dispatch: Dispatch<OrderAction | NotifyAction>) => {
    try {
      const { ozonaToken } = parseCookies();
      const response = await patchAPI(`order/delivered/${id}`, null as any, ozonaToken);
      dispatch({ type: OrderActionTypes.DELIVERED_ORDER, payload: response.data });
    } catch (error: any) {
      dispatch({
        type: NotifyActionTypes.NOTIFY,
        payload: { errors: error.response.data.message },
